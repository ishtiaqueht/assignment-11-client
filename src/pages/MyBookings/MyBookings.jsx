...
const MyBookings = () => {
    ...
    const fetchBookings = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/myBookings?email=${user.email}`, {
          headers: {
            // If using JWT: 'Authorization': `Bearer ${token}`
          },
        });
  ...
  return (
   <div className="max-w-5xl mx-auto p-8 mt-10 bg-gradient-to-br from-gray-900 via-gray-800 to-black rounded-2xl shadow-2xl border border-gray-700">
  <h1 className="text-4xl font-extrabold mb-8 text-center bg-gradient-to-r from-indigo-400 to-cyan-400 text-transparent bg-clip-text">
     My Bookings
  </h1>
  ...
  <div className="overflow-x-auto rounded-2xl shadow-lg">
    <table className="w-full border-collapse">
      <thead>
        <tr className="bg-gradient-to-r from-indigo-600 to-cyan-600 text-white text-lg">
      ...
      <tbody>
        {bookings.map((booking, index) => (
          ...
          >
            ...
            <td className="p-4 text-gray-300">{booking.location}</td>
